/**
 * @license AGPL-3.0
 * Blooket Factory - Choose Any Blook (Fixed)
 * Injects a specific blook into your factory slots
 */
...
(() => {
    ...
    // Full Blook Definition List (from original script)
    const factoryBlooks = [
    ...
    // Scan for the factories container
    ...
    if (targetObj) {
        ...
        const input = iframe.contentWindow.prompt("Enter Blook Name (e.g. Mega Bot, King, Dragon):", "Mega Bot");
        iframe.remove();
        ...
        if (chosenBlook) {
            // Find the first empty slot or replace the last one if full
            const maxSlots = 10; 
            const currentLen = targetObj.factories.length;

            // Clone the blook object
            const newBlook = { ...chosenBlook, level: 4, bonus: 1 }; // Default to level 4 because why not?

            if (currentLen < maxSlots) {
                targetObj.factories.push(newBlook);
                alert(`✅ Added ${chosenBlook.name}!`);
            } else {
                // Replace the last one (or ask user to replace, but this is simpler)
